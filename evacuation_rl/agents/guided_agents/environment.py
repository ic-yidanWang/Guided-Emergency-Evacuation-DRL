...
class GuidedCellSpace(Cell_Space):
    ...
    def initialize_particles(self, file=None):
        """Initialize guided particles with optional guide agents"""
    ...
    def step_guided(self):
        ...
        for c in self.Cells:
            for p in c.Particles:
                ...
                # CHECK FOR WALL/OBSTACLE COLLISION FIRST (but not when near exits!)
                # Disable collision avoidance when agent is close to an exit
                collision_avoid_dir = None
                if exit_dist > self.door_visible_radius:
                    # Only check collisions if agent is far from any exit
                    collision_avoid_dir = self._get_collision_avoidance_direction(p)
                
                # If colliding (and not near exit), prioritize collision avoidance (turn back)
                if collision_avoid_dir is not None:
                    # Override normal behavior when colliding
                    move_vector = collision_avoid_dir
                    desired_speed = desire_velocity * self.speed_scale * 1.2  # Speed up when turning back
                    best_action = self._best_action_for_direction(move_vector)
                
                # Stage 1: Check if influenced by guides
                elif getattr(p, 'is_guide', False) or is_guided_by_agent:
                    # Guide agents or agents influenced by guides → Rush toward exit
                    move_vector = exit_vector
                    desired_speed = desire_velocity * self.speed_scale
                    best_action = self._best_action_for_direction(move_vector)
                # Stage 2: Check if within door_visible_radius
                elif exit_dist <= self.door_visible_radius:
                    # Close to exit → Rush toward exit directly
                    move_vector = exit_vector
                    desired_speed = desire_velocity * self.speed_scale
                    best_action = self._best_action_for_direction(move_vector)
                # Stage 3: Far from exits → Use KNN behavior with strong randomness
                else:
                    if self.use_knn:
                        move_vector, speed_variance = self._knn_direction_and_variance(p, self.knn_k)
                        # Apply noise with AMPLIFIED variance for more realistic uncertainty
                        # Noise is sampled from normal distribution with std=sqrt(variance)
                        noise = np.random.normal(0.0, np.sqrt(speed_variance * 2.0))  # 2x amplification
                        desired_speed = max(0.1, (desire_velocity + noise) * self.speed_scale)
                        best_action = self._best_action_for_direction(move_vector)
                    else:
                        # When use_knn=False, random action
                        action_idx = np.random.choice(len(self.action))
                        best_action = self.action[action_idx]
                        desired_speed = desire_velocity * self.speed_scale
